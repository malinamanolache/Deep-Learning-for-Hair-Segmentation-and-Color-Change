...
class Decoder(nn.Module):
    ...
    def forward(self, skip_features, width, height):

        x_dec_5 = self.dec_5(skip_features[4], skip_features[3])

        x_dec_4 = self.dec_4(x_dec_5, skip_features[2])
        x_dec_3  =self.dec_3(x_dec_4, skip_features[1])
        x_dec_2 = self.dec_2(x_dec_3, skip_features[0])
        self.upsample = nn.Upsample((width, height), mode='bilinear', align_corners=True)
...
class Densenet_Unet(nn.Module):
    def __init__(self):
        ...
        #expanding path
        self.feature_names = ["features.relu0", "features.transition1.pool", "features.transition2.pool", "features.transition3.pool"]
        self.encoder = DenseNet()
        self.encoder = tx.Extractor(self.encoder, self.feature_names)
    ...
    def forward(self, x):
        ...
        prediction = self.decoder(features_list, width, height)
